...
def run_continual_experiment(
    ...
    if optimizer_name == "adamw":
        ...
        optimizer = WorkAdam(
            model.parameters(), lr=lr, beta1=0.9, rho=rho, 
    ...
    for phase in phases:
        ...
        test_datasets[phase.ndigit] = ArithmeticDataset(cfg, split="test", max_ndigit=max_ndigit)
...
def plot_continual_metrics(work_dir: str, phases: list[Phase]):
    ...
    for b in phase_boundaries:
        ax.axvline(b, color="red", linestyle="--", alpha=0.7, label="phase switch" if b == phase_boundaries[0] else "")
    ax.legend()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--optimizer", type=str, default="adamw", choices=["adamw", "workadam"])
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--lr", type=float, default=5e-4)
    parser.add_argument("--rho", type=float, default=0.99, help="WorkAdam: EMA decay for mass")
